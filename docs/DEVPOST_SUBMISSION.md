...
- **Autonomous replanning** (< 2 seconds to replan 450+ routes when traffic spikes)
- **City-specific intelligence** (different rush hours for Amsterdam, Berlin, London)
...
- **Verifiable certificates** for every route with downloadable PDF

### Architecture
Multi-agent autonomous system:
- **Monitor Agent** — Real-time traffic detection + predictive forecasting
- **Router Agent** — A* multi-objective pathfinding + CO₂ quantification
- **Replanner Agent** — Gemini 1.5 Pro chain-of-thought anomaly response
- **API Gateway** — Node.js/Express REST + Socket.IO WebSocket
- **Frontend** — React 19 + interactive 3-city dashboard

### Why We Win
1. **Innovation** — First to combine A* + carbon shadow pricing + predictive anomalies
2. **Scalability** — City-agnostic algorithm extends to unlimited cities
3. **Performance** — 6x faster replanning than OSRM (2 sec vs 12 sec)
4. **Impact** — Monetized carbon value ($111K/year) proven across real EU cities
5. **Autonomy** — Zero human intervention required for continuous optimization
...
- **Same algorithm** — different city parameters (no code changes)
...
- Google Gemini 1.5 Pro (Agentic reasoning)
...
- Autonomous replanning without human intervention
